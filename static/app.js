...
function renderDashboard(data) {
  ...
  const actualMonths = data.monthly.map(x=>x.month);
  const forecastMonths = data.forecast.map(x=>x.month);
  trendChart = new Chart(document.querySelector('#trendChart'), {type:'line', data:{labels:[...actualMonths, ...forecastMonths], datasets:[{label:'Revenue', data:[...data.monthly.map(x=>x.revenue), ...data.forecast.map(x=>x.revenue)], borderColor:'#e88d55', backgroundColor:'rgba(232,141,85,.12)', fill:true, tension:.35, pointRadius:3, pointBackgroundColor:'#fff', pointBorderWidth:2},{label:'Prediction', data:[...data.monthly.map(()=>null), data.monthly.at(-1)?.revenue ?? null, ...data.forecast.map(x=>x.revenue)], borderColor:'#1e5949', borderDash:[6,5], backgroundColor:'transparent', fill:false, tension:.35, pointRadius:3, pointBackgroundColor:'#fff', pointBorderWidth:2}]}, options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{y:{border:{display:false},grid:{color:'#edf0ed'},ticks:{callback:v=>money(v),font:{size:10},color:'#87918b'}},x:{border:{display:false},grid:{display:false},ticks:{font:{size:10},color:'#87918b'}}}}});
  ...
  document.querySelector('#forecastMeta').textContent = `${data.forecast_meta.method} · ${data.forecast_meta.confidence}% confidence`;
  document.querySelector('#forecastCards').innerHTML = data.forecast.map((x, i) => `<div class="forecast-card"><span>${i === 0 ? 'NEXT MONTH' : `MONTH ${i + 1}`}</span><strong>${x.month}</strong><b>${money(x.revenue)}</b><small>projected revenue</small></div>`).join('');
